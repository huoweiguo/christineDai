...
import { getContactUs } from '../../store/modules/contactUs'
...
export default function ContactUs() {
  ...
  useEffect(() => {
    getContactUs().then(res=>{
      if(res.data.code===200){
        ...
        // res.data.data.a.forEach((item:typeData)=>{
        //   preloadImage(item.bg_cover)
        // })
      }
    })
  ...
  return (
    <div className={ styles.contactUs }>
      ...
      { bgData.length > 0 && <div className={ styles['cu-container'] } style={{backgroundImage: 'url('+bgData[activeIndex]?.bg_cover+')'}}>
        { bgData.map((item,index)=>(
          <div className={`${styles.itemBox} ${selectIndex && selectIndex !== 1 ? styles.narrow : ''}`} onMouseEnter={()=>handleMouseEnter(index)}>
            <div className={`${styles.wrapper} ${selectIndex === 1 ? styles.showWrapper : ''}`}>
              ...
              <div className={`${styles.content } ${selectIndex === 1 ? styles.showContent : ''}`}>
                ...
                <p className={styles['c-btn']} onClick={()=>openContactForm(1)}>{item.contact}</p>
              </div>
            </div>
          </div>
        ))
        ...
        <div className={`${styles['contact-form']} ${selectIndex ? styles.enlarge : (selectIndex === 0 ? styles['enlarge-leave'] : '')}`}>
          ...
          <Row gutter={16}>
            ...
            <Col span={24} className={styles.cbox}>
              ...
              <span className={`${styles['c-btn']} ${styles.sub}`}>发送</span>
            </Col>
          </Row>
          </Form>
        </div>
